...
class OpenAIVisionLLM(BaseVisionLLM):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.thinking = (
            getattr(vision_settings, "thinking", None)
            if vision_settings
            else getattr(settings.llm, "thinking", None)
        )
        ...
        # Vision can use a different OpenAI-compatible provider than text
        # chat (for example, DeepSeek's vision endpoint).  Honour its URL
        # before falling back to the text LLM's Azure configuration or OpenAI.
        vision_base_url = (
            getattr(vision_settings, 'base_url', None)
            if vision_settings
            else None
        )
