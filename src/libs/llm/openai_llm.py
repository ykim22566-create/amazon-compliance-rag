...
class OpenAILLM(BaseLLM):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.thinking = getattr(settings.llm, "thinking", None)
        ...
        # Resolution priority: explicit constructor arg > settings.llm.base_url
        # > settings.llm.azure_endpoint > DEFAULT_BASE_URL.  The explicit
        # settings.llm.base_url branch was missing previously, so any proxy
        # URL configured in settings.yaml was silently ignored and requests
        # went to api.openai.com with the proxy key (→ 401).
