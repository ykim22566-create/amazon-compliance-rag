...
class ImageCaptioner(BaseTransform):
    ...
    def _get_caption(
        ...
        # Validate path
        resolved_img_path = Path(img_path) if img_path else None
        if resolved_img_path and not resolved_img_path.exists():
            resolved_img_path = resolve_path(img_path)

        if not resolved_img_path or not resolved_img_path.exists():
            logger.warning(f"Image path not found: {img_path}")
            return None
        
        try:
            image_input = ImageInput(path=str(resolved_img_path))
