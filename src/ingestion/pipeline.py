...
class IngestionPipeline:
    ...
    def run(
        ...
        Args:
            ...
            source_path_override: Optional stable source name/path to store in
                chunk metadata when the physical file is a temporary upload.
        
        Returns:
            PipelineResult with success status and statistics
        """
        file_path = Path(file_path)
        source_path_for_metadata = source_path_override or str(file_path)
        stages: Dict[str, Any] = {}
        _total_stages = 6
        ...
        try:
            ...
            if not self.force and self.integrity_checker.should_skip(file_hash):
                ...
                return PipelineResult(
                    success=True,
                    file_path=source_path_for_metadata,
                    doc_id=file_hash,
                    stages={"integrity": {"skipped": True, "reason": "already_processed"}}
                )

            if self.force:
                cleanup = self._cleanup_existing_document(
                    file_hash, source_path_for_metadata
                )
                stages["force_cleanup"] = cleanup
                if trace is not None:
                    trace.record_stage("force_cleanup", cleanup)
            ...
            document.metadata["source_path"] = source_path_for_metadata
            ...
            logger.info(f"  Sparse stats: {len(sparse_stats)} chunks")
            ...
            storage_prefix = storage_chunk_prefix(source_path_for_metadata)
            ...
            logger.info(f"      Index built for {len(sparse_stats)} chunks")
            ...
            for img in images:
                ...
                if not img_path.exists():
                    img_path = resolve_path(img["path"])
                if img_path.exists():
            ...
            self.integrity_checker.mark_success(
                file_hash, source_path_for_metadata, self.collection
            )
            ...
            self.integrity_checker.mark_failed(file_hash, source_path_for_metadata, str(e))
    ...
    def _cleanup_existing_document(
        self, file_hash: str, source_path: str
    ) -> Dict[str, Any]:
        """Remove previous records for a document before forced reprocessing."""
        ...
        try:
            cleanup["chunks_deleted"] = self.vector_upserter.vector_store.delete_by_metadata(
                {"doc_hash": file_hash}
            )
        except Exception as e:
            cleanup["errors"].append(f"ChromaDB cleanup failed: {e}")

        storage_prefix = storage_chunk_prefix(source_path)
        cleanup["bm25_prefix"] = storage_prefix
        try:
            cleanup["bm25_removed"] = self.bm25_indexer.remove_document(
                storage_prefix, self.collection
            )
            if not cleanup["bm25_removed"]:
                logger.warning(
            ...
            cleanup["errors"].append(f"BM25 cleanup failed: {e}")

        try:
            old_images = self.image_storage.list_images(doc_hash=file_hash)
            deleted_images = 0
            for img in old_images:
                if self.image_storage.delete_image(img["image_id"]):
                    deleted_images += 1
            cleanup["images_deleted"] = deleted_images
        except Exception as e:
            cleanup["errors"].append(f"Image cleanup failed: {e}")

        if cleanup["errors"]:
            logger.warning("Force cleanup completed with errors: %s", cleanup["errors"])
        else:
            logger.info(
                "Force cleanup removed %s chunks, %s images",
                cleanup["chunks_deleted"],
                cleanup["images_deleted"],
            )
