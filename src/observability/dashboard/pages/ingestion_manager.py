...
def _run_ingestion(
    ...
    try:
        ...
        result = pipeline.run(
            ...
            source_path_override=uploaded_file.name,
        )
        if not result.success:
            if trace.get_stage_data("pipeline_error") is None:
                trace.record_stage(
                    "pipeline_error",
                    {
                        "error": result.error or "Pipeline failed without an error message.",
        ...
        trace.record_stage("pipeline_error", {"error": str(exc)})
